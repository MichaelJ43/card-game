...
import type { ApplyResult } from '../../core/gameModule'
...
import { cmd, compare5, stacksFrom, totalPlayers } from './helpers'
import type { PokerDrawGameState } from './types'

export const pokerDrawLogic: Pick<
  GameModule<PokerDrawGameState>,
...
> = {
  setup(ctx, instances) {
    ...
    return {
      ...
      gameState: {
        ...
        roundDelta: null,
        message:
          'Ante 10 chips. Then replace 0–3 cards from the front of your hand (same count for both). Highest rank wins the pot.',
  ...
  applyAction(table, gs, action): ApplyResult<PokerDrawGameState> {
    ...
    if (gs.phase === 'draw' && c === 'p5Draw') {
      ...
      const p0 = t.zones['hand:0']!.cards.map((c: CardInstance) => c.templateId)
      const p1 = t.zones['hand:1']!.cards.map((c: CardInstance) => c.templateId)
